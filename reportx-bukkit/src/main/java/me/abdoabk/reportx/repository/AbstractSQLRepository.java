...
public abstract class AbstractSQLRepository implements ReportRepository {
    ...
    protected final ReportXPlugin plugin;
    protected final ExecutorService executor = Executors.newFixedThreadPool(4);

    protected AbstractSQLRepository(ReportXPlugin plugin, Logger logger) {
        this.plugin = plugin;
        this.logger = logger;
    }
    ...
    public Connection getConnectionPublic() throws SQLException {
    ...
    private void runMigrations(Connection conn) {
        addColumnIfMissing(conn, "reports",   "reporter_name",      "VARCHAR(32)");
        addColumnIfMissing(conn, "reports",   "accused_name",       "VARCHAR(32)");
        addColumnIfMissing(conn, "reports",   "chat_snapshot",      "LONGTEXT");
        addColumnIfMissing(conn, "reports",   "inventory_snapshot", "LONGTEXT");
        addColumnIfMissing(conn, "reports",   "reporter_ip",        "VARCHAR(50)");
        addColumnIfMissing(conn, "reports",   "claimed_by_name",    "VARCHAR(32)");
        // Add staff_name to notes and audit_log so names are stored at write time
        addColumnIfMissing(conn, "notes",     "staff_name",         "VARCHAR(32)");
        addColumnIfMissing(conn, "audit_log", "staff_name",         "VARCHAR(32)");
    }
    ...
    public CompletableFuture<Long> saveReport(Report report) {
        return CompletableFuture.supplyAsync(() -> {
            String sql = "INSERT INTO reports " +
                    "(reporter_uuid, reporter_name, accused_uuid, accused_name, reason, status, world, x, y, z, created_at, chat_snapshot, inventory_snapshot, reporter_ip) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            try (Connection conn = getConnection();
                ...
                ps.setString(3,  report.getAccusedUuid().toString());
                ps.setString(4,  report.getAccusedName());
                ...
                ps.setTimestamp(11, Timestamp.valueOf(report.getCreatedAt()));
                ps.setString(12, report.getChatSnapshot());
                ps.setString(13, report.getInventorySnapshot());
                ps.setString(14, report.getReporterIp());
    ...
    public CompletableFuture<Boolean> updateReport(Report report) {
        return CompletableFuture.supplyAsync(() -> {
            String sql = "UPDATE reports SET status = ?, claimed_by = ?, claimed_by_name = ?, resolved_at = ?, verdict = ? WHERE id = ?";
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(sql)) {

                ps.setString(1, report.getStatus().name());
                ps.setString(2, report.getClaimedBy() != null ? report.getClaimedBy().toString() : null);
                ps.setString(3, report.getClaimedByName());
                ps.setTimestamp(4, report.getResolvedAt() != null ? Timestamp.valueOf(report.getResolvedAt()) : null);
                ps.setString(5, report.getVerdict());
    ...
    public CompletableFuture<Long> saveNote(Note note) {
        return CompletableFuture.supplyAsync(() -> {
            // staff_name stored directly — no lookup needed at read time
            String sql = "INSERT INTO notes (report_id, staff_uuid, staff_name, note, created_at) VALUES (?, ?, ?, ?, ?)";
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                ps.setLong(1,   note.getReportId());
                ps.setString(2, note.getStaffUuid().toString());
                ps.setString(3, note.getStaffName());
                ps.setString(4, note.getNote());
                ps.setTimestamp(5, Timestamp.valueOf(note.getCreatedAt()));
            ...
            try (Connection conn = getConnection();
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        note.setStaffName(rs.getString("staff_name"));
    ...
    public CompletableFuture<Void> saveAuditLog(AuditLog auditLog) {
        return CompletableFuture.runAsync(() -> {
            // staff_name stored directly — no lookup needed at read time
            String sql = "INSERT INTO audit_log (staff_uuid, staff_name, action, timestamp) VALUES (?, ?, ?, ?)";
            try (Connection conn = getConnection();
                 PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, auditLog.getStaffUuid().toString());
                ps.setString(2, auditLog.getStaffName());
    ...
    protected Report mapReport(ResultSet rs) throws SQLException {
        ...
        report.setClaimedByName(rs.getString("claimed_by_name"));
        ...
        report.setAccusedName(rs.getString("accused_name"));
