...
public class StaffReportGUI {
    ...
    private ItemStack createReportItem(Report report) {
        ...
        String accusedName = report.getAccusedName() != null ? report.getAccusedName() : getPlayerName(report.getAccusedUuid());
        String reporterName = report.getReporterName() != null ? report.getReporterName() : getPlayerName(report.getReporterUuid());

        // Display name (UUID) if name is available, otherwise just the name/UUID
        String accusedDisplay = report.getAccusedName() != null ? accusedName + " (" + report.getAccusedUuid().toString().substring(0, 8) + ")" : accusedName;
        String reporterDisplay = report.getReporterName() != null ? reporterName + " (" + report.getReporterUuid().toString().substring(0, 8) + ")" : reporterName;

        String name = plugin.getMessageUtil().get("gui.report-item-name", Map.of(
                "id", String.valueOf(report.getId()),
                "accused", accusedDisplay
        ));
